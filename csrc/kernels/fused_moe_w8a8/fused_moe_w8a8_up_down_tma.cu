...
__global__ __launch_bounds__(WN*32 + PRODUCER_THREADS) void fused_moe_w8a8_wgmma_up_down_tma_kernel(
            ...
            for(int tn2 = 0; tn2<TN2; tn2+=2)
            {
                for(int tm = 0; tm<TM; tm++)
                {
                    ...
                    for (int t = 0; t<8; t++)
                    {
                        ...
                        {
                            int s = t%2;
                            acc += token_max[tm][s]*tile_acc[tn2 + t/4][tm][t%4];
                        }
                        tile[t] = acc*scale_w[tn2/2];
                    }
...
template<int BM>
void launch_fused_moe_kernel_up_down_tma(
        const fp8* x,
        const float* x_scale,
        ...
        )
{
    ...
    gpuErrchk(cudaFuncSetAttribute(
                fused_moe_w8a8_wgmma_up_down_tma_kernel<BM, BK, BN, WN, STAGES, PRODUCER_THREADS>,
    ...
    fused_moe_w8a8_wgmma_up_down_tma_kernel<BM, BK, BN, WN, STAGES, PRODUCER_THREADS><<<dimGrid, dimBlock, sMemSize>>>(
            x,
            x_scale,
            tensor_map_w,
            w_scale,
...
void fused_moe_w8a8_wgmma_up_down_tma(
        const fp8* x,
        const float* x_scale,
        fp8* w, const float* w_scale,
        fp8* w2, const float* w2_scale,
...
{
    switch(block_m) {
        case 8:   launch_fused_moe_kernel_up_down_tma<8>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids,   num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 16:  launch_fused_moe_kernel_up_down_tma<16>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids,  num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 24:  launch_fused_moe_kernel_up_down_tma<24>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids,  num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 32:  launch_fused_moe_kernel_up_down_tma<32>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids,  num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 40:  launch_fused_moe_kernel_up_down_tma<40>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids,  num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 48:  launch_fused_moe_kernel_up_down_tma<48>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids,  num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 56:  launch_fused_moe_kernel_up_down_tma<56>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids,  num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 64:  launch_fused_moe_kernel_up_down_tma<64>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids,  num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 72:  launch_fused_moe_kernel_up_down_tma<72>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids,  num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 80:  launch_fused_moe_kernel_up_down_tma<80>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids,  num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 88:  launch_fused_moe_kernel_up_down_tma<88>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids,  num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 96:  launch_fused_moe_kernel_up_down_tma<96>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids,  num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 104: launch_fused_moe_kernel_up_down_tma<104>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 112: launch_fused_moe_kernel_up_down_tma<112>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 120: launch_fused_moe_kernel_up_down_tma<120>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        case 128: launch_fused_moe_kernel_up_down_tma<128>(x, x_scale, w, w_scale, w2, w2_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, topk_weights, top_k, M, K, N, sorted_num, block_m); break;
        default:
