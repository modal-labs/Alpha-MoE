...
constexpr __device__ int32_t const_ceil(float num)
{
...
__device__ inline uint64_t matrix_descriptor_encode(uint64_t x) {
    return (((x) & 0x3FFFF) >> 4);
}
...
__device__ __forceinline__ uint64_t make_smem_desc(fp8* ptr, int sdo, int ldo=1) {
...
{
    if constexpr (BM == 8)
        wgmma8<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 16)
        wgmma16<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 24)
        wgmma24<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 32)
        wgmma32<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 40)
        wgmma40<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 48)
        wgmma48<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 56)
        wgmma56<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 64)
        wgmma64<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 72)
        wgmma72<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 80)
        wgmma80<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 88)
        wgmma88<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 96)
        wgmma96<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 104)
        wgmma104<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 112)
        wgmma112<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 120)
        wgmma120<ScaleD, ScaleA, ScaleB>(d, sA, sB);
    else if constexpr (BM == 128)
        wgmma128<ScaleD, ScaleA, ScaleB>(d, sA, sB);
}
...
template<int STAGES, int WS, int XS>
struct smem
{
...
__global__ __launch_bounds__(WN*32 + PRODUCER_THREADS) void fused_moe_w8a8_wgmma_tma_swiglu_kernel(
        ...
        )
{
    ...
    smem<STAGES, WS, XS>& s = *reinterpret_cast<smem<STAGES, WS, XS>*>(sh);
    ...
    if (is_producer)
    {
        const int w_row = exp_idx * N + (blockIdx.x)*WN*BN + (lane_id>>2);
        ...
        for (int load_stage = 0; load_stage<n_stages; load_stage++)
        {
            ...
            if(threadIdx.x == 0)
            {
                ...
                load_async(s.w + smem_stage*WS, &tensor_map_w, bar + smem_stage, w_row, off);
            }
        }
    }
    // CONSUMER
    ...
    {
        const int w_row = exp_idx * N + warpN*BN + (lane_id>>2);
        ...
        for (int compute_stage = 0; compute_stage < n_stages; compute_stage += 1)
        {
            ...
            // float scale_w = w_scale[(w_row/block_shape[1])*scale_cols_w + compute_stage];
            float scale_w[2];
            int s_r = (exp_idx * N)/(block_shape[1]) + ((lane_id>>2) + warpN*BN)/(block_shape[1]*2);
            scale_w[0] = w_scale[(s_r)*scale_cols_w + compute_stage];
            scale_w[1] = w_scale[(s_r + 1)*scale_cols_w + compute_stage];
            int stage = compute_stage%STAGES;
            ...
            for(int tn = 0; tn<TN; tn++)
            {
                ...
                wgmma<1,1,1, BM>(tile_acc[tn], sw+3*32, sx+3*32);
            }
...
template<int BM>
void launch_fused_moe_kernel_tma_swiglu(
        const fp8* x,
        const float* x_scale,
        fp8* w,
        const float* w_scale,
        ...
        )
{
    ...
    size_t sMemSize = sizeof(smem<STAGES, WN*BK*BN, BK*BM>);
    gpuErrchk(cudaFuncSetAttribute(
        fused_moe_w8a8_wgmma_tma_swiglu_kernel<BM, BK, BN, WN, STAGES, PRODUCER_THREADS>,
    ...
    fused_moe_w8a8_wgmma_tma_swiglu_kernel<BM, BK, BN, WN, STAGES, PRODUCER_THREADS><<<dimGrid, dimBlock, sMemSize>>>(
            x,
            x_scale,
            // w,
            tensor_map,
...
void fused_moe_w8a8_wgmma_tma_swiglu(
        const fp8* x,
        const float* x_scale,
...
{
    switch(block_m) {
        case 8:   launch_fused_moe_kernel_tma_swiglu<8>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 16:  launch_fused_moe_kernel_tma_swiglu<16>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 24:  launch_fused_moe_kernel_tma_swiglu<24>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 32:  launch_fused_moe_kernel_tma_swiglu<32>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 40:  launch_fused_moe_kernel_tma_swiglu<40>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 48:  launch_fused_moe_kernel_tma_swiglu<48>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 56:  launch_fused_moe_kernel_tma_swiglu<56>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 64:  launch_fused_moe_kernel_tma_swiglu<64>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 72:  launch_fused_moe_kernel_tma_swiglu<72>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 80:  launch_fused_moe_kernel_tma_swiglu<80>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 88:  launch_fused_moe_kernel_tma_swiglu<88>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 96:  launch_fused_moe_kernel_tma_swiglu<96>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 104: launch_fused_moe_kernel_tma_swiglu<104>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 112: launch_fused_moe_kernel_tma_swiglu<112>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 120: launch_fused_moe_kernel_tma_swiglu<120>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        case 128: launch_fused_moe_kernel_tma_swiglu<128>(x, x_scale, w, w_scale, out, sorted_token_ids, expert_ids, num_tokens_post_padded, top_k, M, K, N, sorted_num, block_m); break;
        default:
